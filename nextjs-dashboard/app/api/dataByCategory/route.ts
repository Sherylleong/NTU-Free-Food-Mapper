...
import { CategoryDataRow, queryFiltersProcessedDataCategoryStatistics } from '../../helpers/db_helper';
...
export async function POST(req: Request) {
  try {
    ...
    const dataByCategory: CategoryDataRow[] = await queryFiltersProcessedDataCategoryStatistics(filters);

    // Return the filtered data as JSON
    return NextResponse.json(dataByCategory, { status: 200 });
  } catch (error: any) {
