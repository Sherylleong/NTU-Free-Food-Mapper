...
async def scrape_tele_all(chat, client, startdate=EARLIEST_DATE):
    ...
    sql_insert_processed_data = """
            INSERT INTO PROCESSED_DATA (min_id, max_id, msg_first_date, msg_last_date, msg_sender, location, categories, msg_text, time_to_clear)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """
    sql_update_metadata = """
            INSERT INTO METADATA (max_id, msg_last_date, latest_update_time)
            VALUES (?, ?, ?)
            """
    try:
        ...
        for record in df:
            ...
            msg_text = record['text']
            time_to_clear = record['time_to_clear']
            cursor.execute(sql_insert_processed_data, (min_id, max_id, msg_first_date, msg_last_date, msg_sender, location, categories, msg_text, time_to_clear))
        
        cursor.execute(sql_update_metadata, (df['min_id'].max(), df['msg_last_date'].max(), datetime.datetime.now().astimezone(sgt)))
...
def combine_msg_blocks(df):
    ...
    df = df[['min_id', 'max_id', 'msg_first_date', 'msg_last_date', 'sender', 'location', 'text', 'time_to_clear']]
    return df
